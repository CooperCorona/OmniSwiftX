...
public class GLSFrameSprite: GLSSprite {
    ...
    public init?(sizes:[NSSize], textures:[CCTexture?]) {
        
        self.sizes      = sizes
        self.textures   = textures
        
        let initialSize = self.sizes.count > 0 ? self.sizes[0] : NSSize.zero
        let initialTex  = self.textures.count > 0 ? self.textures[0] : nil
        
        super.init(position: NSPoint.zero, size: initialSize, texture: initialTex)
    ...
    public convenience init!(size:NSSize, textures:[CCTexture?]) {
...
//                    self.textureIndex   = self.textures.count - 1
    ...
    private func textureIndexChanged() -> Bool {
        ...
        TexturedQuad.setPosition(NSRect(x: 0.0, y: 0.0, width: self.contentSize.width, height: self.contentSize.height), ofVertices: &self.vertices)
