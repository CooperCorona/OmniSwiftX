...
public class GLSDoubleSprite: GLSSprite, DoubleBuffered {
    ...
    public init(size:NSSize, firstTexture:CCTexture?, secondTexture:CCTexture?) {
        
        self.buffer = GLSFrameBuffer(size: size)
        
//        super.init(position: NSPoint.zero, size: size)
//        super.init(size: size, texture: firstTexture)
        super.init(position: NSPoint.zero, size: size, texture: firstTexture)
    ...
    override public func setQuadForTexture() {
        ...
        let frame1 = self.firstTexture?.frame  ?? NSRect(x: 0, y: 0, width: 1, height: 1)
        let frame2 = self.secondTexture?.frame ?? NSRect(x: 0, y: 0, width: 1, height: 1)
        let frame1SizeAsPoint = frame1.size.getNSPoint()
        let frame2SizeAsPoint = frame2.size.getNSPoint()
