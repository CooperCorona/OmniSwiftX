...
public class GLSTileSprite: GLSSprite {
    ...
    public var textureSize:NSSize = NSSize(square: 1.0) {
        didSet {
    ...
    public init(size:NSSize, textureSize:NSSize, texture:CCTexture?) {
        self.textureSize = textureSize
        super.init(position: NSPoint.zero, size: size, texture: texture)
        self.setTextureOfVertices()
    }
