...
public class GLSNode: NSObject {
    ...
    public var position:NSPoint = NSPoint(x: 0.0, y: 0.0) {
    ...
    public var velocity:NSPoint = NSPoint(x: 0.0, y: 0.0) {
    ...
    public var anchor:NSPoint = NSPoint(x: 0.5, y: 0.5) {
        didSet {
    ...
    public var contentSize = NSSize(width: 0.0, height: 0.0) {
        didSet {
            self.contentSizeChanged()
        }
    }
    public var frame:NSRect {
        let realSize = self.contentSize * NSSize(width: self.scaleX, height: self.scaleY)
        ...
        return NSRect(origin: NSPoint(x: xpos, y: ypos), size: realSize)
    }
    ...
    public var projection:SCMatrix4 {
        get {
            if let proj = self.internalProjection ?? self.recursiveRenderer?.projection {
                return proj
            } else {
//                print("Invalid Projection!")
    ...
    public var universalRenderIndex:Int = -1
    public var removeAtUpdate = false
    public var removeFromUniversalRenderer = false
    public var hidden = false
    public var modelMatrixAlwaysUsesRenderSelf = false
    public weak var recursiveRenderer:RecursiveRenderer? = nil
    ...
    override convenience init() {
        self.init(position:NSPoint(x: 0.0, y: 0.0), size:NSSize(width: 0.0, height: 0.0))
    }//default initialization
    
    public init(position:NSPoint, size:NSSize) {
    ...
    public convenience init(frame:NSRect, projection:SCMatrix4) {
    ...
    public func addChild(child:GLSNode) {
        ...
        let block:() -> Void = { [weak self] in
            ...
            self?.nodeAddedAsChild_Recursive(child)
        }
        
        if let r = self.recursiveRenderer {
            let op = NSBlockOperation(block: block)
            op.name = "Insert (\(child.title))"
            r.addOperation(op)
        } else {
            block()
        }
    ...
    public func removeChild(child:GLSNode) -> GLSNode? {
        ...
        let block:() -> Void = { [unowned self] in
            if child.superNode === self {
                child.superNode = nil
                
                self.nodeRemovedFromParent_Recursive(child)
                
                removeObject(child, fromArray:&self.children)
            }
        }
        
        if let r = self.recursiveRenderer {
            let op = NSBlockOperation(block: block)
            op.name = "Remove (\(child.title))"
            r.addOperation(op)
        } else {
            block()
        }
        ...
        self.nodeRemovedFromParent(child)
...
public extension GLSNode {
    ...
    public func allGlErrors(handler:([GLenum]) -> Void) {
        var errors:[GLenum] = []
