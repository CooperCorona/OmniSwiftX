...
float getDotAtIndex(int index, vec3 offset) {
    ...
    vec3 noiseTex = texture(u_NoiseTextureInfo, vec2(v_x, 0.0)).rgb;
    //    vec3 noiseTex = texture(u_PermutationInfo,  vec2(v_x, 0.0)).gba;
...
int permAtIndex(int index) {
    ...
    vec4 texVal = texture(u_PermutationInfo, vec2(float(index) / 255.0, 0.0));
    //    vec4 texVal = texture(u_PermutationInfo, v_Texture);
    return int(texVal.x * 255.0);
}

ivec4 ivecPermAtIndex(int index) {
    highp vec4 col = texture(u_PermutationInfo, vec2(float(index) / 255.0, 0.0));
...
void main(void) {
    ...
    c_gl_FragColor = vec4(graColor.rgb * texColor.rgb, graColor.a * texColor.a);
}//main
