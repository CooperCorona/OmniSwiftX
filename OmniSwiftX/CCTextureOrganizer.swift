...
public class CCTextureOrganizer: NSObject, XMLFileHandlerDelegate {
    ...
    public func processAtlasBatch(attributes:[NSObject : AnyObject]) {
        ...
        let size = NSSize(width: dataComps[2].getCGFloatValue(), height: dataComps[3].getCGFloatValue())
...
//        let buffer = (attributes["buffer")?.getCGFloatValue() ?? 0.0
        ...
        for jjj in 0..<yIterations {
            ...
            for iii in 0..<xIterations {
                ...
                //let frame = NSRect(x: width * iii, y: height * jjj, width: width, height: height)
                let frame = NSRect(x: width * CGFloat(iii), y: height * CGFloat(jjj), width: width, height: height)
    ...
    public class func createTexture(file:String, fileExtension:String, size:NSSize) -> GLKTextureInfo {
    ...
    public class func createPNGTexture(file:String, size:NSSize) -> GLKTextureInfo {
...
//        let path = self.pathForPNG(file)
    ...
    public class var sharedContext:NSOpenGLContext {
        
        struct StaticInstance {
            static var instance:NSOpenGLContext! = nil
            static var onceToken:dispatch_once_t = 0
        }
        
        dispatch_once(&StaticInstance.onceToken) {
            StaticInstance.instance = NSOpenGLContext(format: NSOpenGLPixelFormat(), shareContext: nil)
        }
