...
public class NoiseArray2D {
    ...
    public private(set) var gradients = [NSPoint](count: NoiseArray2D.arrayCount, repeatedValue: NSPoint.zero)
    ...
    public init(seed:UInt32 = 1) {
        ...
        for iii in 0..<NoiseArray2D.totalCount {
            ...
            let p = NSPoint(angle: angle, length: 1.0)
    ...
    */
    public func noiseAt(vec:NSPoint) -> NoiseType {
        ...
        */
        let offsetLL = NSPoint(x: xComponents.preDistance,  y: yComponents.preDistance)
        let offsetUL = NSPoint(x: xComponents.postDistance, y: yComponents.preDistance)
        let offsetLU = NSPoint(x: xComponents.preDistance,  y: yComponents.postDistance)
        let offsetUU = NSPoint(x: xComponents.postDistance, y: yComponents.postDistance)
        let offsets = [
            offsetLL,
            offsetUL,
            offsetLU,
    ...
    */
    public func positiveNoiseAt(vec:NSPoint) -> NoiseType {
