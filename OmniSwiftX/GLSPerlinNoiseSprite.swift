...
//    public private(set) var noiseProgram = PerlinNoiseProgram(type: .Default)
    public private(set) var noiseProgram = ShaderHelper.programDictionaryForString("Perlin Noise Shader")!
...
//    public private(set) lazy var buffer:GLSFrameBuffer = GLSFrameBuffer(size: self.contentSize)
    ...
    public var noiseType:NoiseType = NoiseType.Default {
        didSet {
            ...
            case .Default:
                self.noiseProgram = ShaderHelper.programDictionaryForString("Perlin Noise Shader")!
            case .Fractal:
                self.noiseProgram = ShaderHelper.programDictionaryForString("Perlin Fractal Noise Shader")!
            case .Abs:
    ...
    */
    ...
    public init(size:NSSize, texture:CCTexture?, noise:Noise3DTexture2D, gradient:GLGradientTexture2D) {
        ...
        self.noiseVertices.iterateWithHandler() { index, vertex in
            ...
            vertex.aspectRatio = (curPoint * NSPoint(x: 1.0, y: size.height / size.width)).getGLTuple()
            return
        }
...
//        glUniform1i(self.noiseProgram.u_PermutationInfo, 3)
        ...
        */
        ...
        self.noiseProgram.uniform1f("u_NoiseDivisor", value: self.noiseDivisor)
        self.noiseProgram.uniform1f("u_Alpha", value: self.noiseAlpha)
        glUniform3i(self.noiseProgram["u_Period"], GLint(self.period.x), GLint(self.period.y), GLint(self.period.z))
        if (self.noiseType == .Sin) {
            self.noiseProgram.uniform1f("u_NoiseAngle", value: self.noiseAngle)
        }
        
//        self.noiseProgram.attributeBridger.enableAttributes()
//        self.noiseProgram.attributeBridger.bridgeAttributesWithSizes([2, 2, 3], stride: sizeof(PerlinNoiseVertex))
        self.noiseProgram.enableAttributes()
        self.noiseProgram.bridgeAttributesWithStride(self.noiseVertices.stride)
