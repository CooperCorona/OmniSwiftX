...
public class GLSAlphaHoleSprite: GLSSprite, DoubleBuffered {
    ...
    public var fadePosition:NSPoint     = NSPoint.zero {
        didSet {
    ...
    public var alphaTexture:CCTexture?  = nil {
        didSet {
            if let tFrame = self.alphaTexture?.frame {
                self.alphaVertices.alterWithFrame(tFrame) { (point:NSPoint, inout vertex:UVertex) in
    ...
    public init(size:NSSize, texture:CCTexture?) {
        ...
        super.init(position: NSPoint.zero, size: size, texture: self.buffer.ccTexture)
