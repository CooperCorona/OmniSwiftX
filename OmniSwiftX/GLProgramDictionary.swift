...
public class GLProgramDictionary: GLAttributeBridger {
    ...
    */
    ...
    public func bindTexture(texture:GLuint?) {
        glBindTexture(GLenum(GL_TEXTURE_2D), texture ?? 0)
        glUniform1i(self["u_TextureInfo"], 0)
    }
    
    public func bridgeAttributesWithStride(stride: Int) -> Bool {
        guard let sizes = self.attributeSizes else {
            return false
        }
        super.bridgeAttributesWithSizes(sizes, stride: stride)
        return true
    }
    
    ///Invokes glUniform1f with the given location and value.
    ...
    public func uniform2f(location:String, value:NSPoint) {
